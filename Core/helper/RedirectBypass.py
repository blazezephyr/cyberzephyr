...
try:
    from .color import green, white, red, start, alert, numbering
except ImportError as e:
    logging.error(f"Failed to import color module: {e}")
    print(f"[!] Error: Color module not found - {e}")
    sys.exit(1)
...
def RedirectHelp():
	try:
		...
		OptionPick = int(input(green + "root@BlazeZephyr/Bypass/Redirect:~ " + white))
		...
		logger.error("Invalid input in RedirectHelp - not a number")
		print("\n[!] Error: Please enter a valid number")
	except KeyboardInterrupt:
		logger.info("User interrupted RedirectHelp")
		print("\nOperation cancelled by user")
	except EOFError:
		logger.error("EOF encountered in RedirectHelp")
		print("\n[!] Error: Input interrupted")
	except Exception as e:
		logger.error(f"Error in RedirectHelp: {e}")
		print(f"[!] Error: {e}")
...
-------------------MainMenu------------------""")
		...
		logger.error(f"Error in RedirectionMain: {e}")
		print(f"[!] Error: {e}")
...
-------------------Creator-------------------""")
	...
	Creator = int(input(green + "root@BlazeZephyr/Redirect/Creator:~ " + white))
	
	if Creator == 1:
		Url = input(start + "Enter Your Url: " + white)
		FileName = input(start + "Name Of File: " + white)
		print(alert + " Enter for '/root/Desktop/BlazeZephyr/Redirection/'")
		FileSave = input(start + " Where Do You Want To Save The File?: " + white)
		if FileSave == "":
			 FileLocation = "/root/Desktop/BlazeZephyr/Redirection/"
		else:
...
----------------Info And HowTo---------------""")
	...
	OptionPick = int(input(green + "root@CyberPhish/Bypass/Redirect/Help:~ " + white))
