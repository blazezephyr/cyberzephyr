...
def IpPick():
	...
	try:
		...
		IpPick = int(input(green + "root@BlazeZephyr/Ip:~ "))
		print("")
		
	except ValueError:
		logger.error("Invalid input in IpPick - not a number")
		print("\n[!] Error: Please enter a valid number")
		return
	except KeyboardInterrupt:
		logger.info("User interrupted IpPick function")
		print("\nOperation cancelled by user")
		return
	except EOFError:
		logger.error("EOF encountered in IpPick function")
		print("\n[!] Error: Input interrupted")
		return
	except Exception as e:
		logger.error(f"Error in IpPick input: {e}")
		print(f"[!] Error: {e}")
		return
	...
	try:
		if IpPick == 1:
			try:
				...
				if not GenIp.strip():
					print(alert + " [!] Error: IP address cannot be empty")
					return
				
				# Basic IP validation
				ip_pattern = r'^(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$'
				if not re.match(ip_pattern, GenIp.strip()):
					print(alert + " [!] Error: Invalid IP address format")
					return
					
				Country = input(start + " Enter Country (might not be used): ")
				City = input(start + " Enter City (might not be used): ")
			except KeyboardInterrupt:
				logger.info("User interrupted custom IP input")
				print("\nOperation cancelled by user")
				return
			except EOFError:
				logger.error("EOF encountered during custom IP input")
				print("\n[!] Error: Input interrupted")
				return
			except Exception as e:
				logger.error(f"Error in custom IP input: {e}")
				print(f"[!] Error: {e}")
				return
			...
			try:
				...
				PreIp = int(input(green + "root@BlazeZephyr/IpPick:~  "))
				...
				logger.error("Invalid input in PreIp selection - not a number")
				print("\n[!] Error: Please enter a valid number")
				return
			except KeyboardInterrupt:
				logger.info("User interrupted PreIp selection")
				print("\nOperation cancelled by user")
				return
			except EOFError:
				logger.error("EOF encountered during PreIp selection")
				print("\n[!] Error: Input interrupted")
				return
			except Exception as e:
				logger.error(f"Error in PreIp selection: {e}")
				print(f"[!] Error: {e}")
				return
		else:
			print(alert + "Invalid Input")
		...
		logger.error(f"Error in IpPick function: {e}")
		print(f"[!] Error: {e}")
		return
