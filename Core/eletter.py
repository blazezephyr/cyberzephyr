...
def Instagram():
	try:
		...
		if not all([Target.strip(), TargetAccount.strip(), url.strip(), TargetEmail.strip()]):
			print(alert + " [!] Error: All fields are required")
			return
		
		if not url.startswith(('http://', 'https://')):
			print(alert + " [!] Error: URL must start with http:// or https://")
			return
			
	except KeyboardInterrupt:
		logger.info("User interrupted Instagram function")
		print("\nOperation cancelled by user")
		return
	except EOFError:
		logger.error("EOF encountered in Instagram function")
		print("\n[!] Error: Input interrupted")
		return
	except Exception as e:
		logger.error(f"Error in Instagram input: {e}")
		print(f"[!] Error: {e}")
		return
	...
	try:
		...
		if not filename.strip():
			print(alert + " [!] Error: Filename cannot be empty")
			return
		
		if not filename.replace('_', '').replace('-', '').isalnum():
			print(alert + " [!] Error: Filename should contain only letters, numbers, underscores, and hyphens")
			return
		...
		logger.info("User interrupted Instagram file creation")
		print("\nOperation cancelled by user")
		return
	except IOError as e:
		logger.error(f"File I/O error in Instagram: {e}")
		print(f"[!] Error: Could not create file - {e}")
		return
	except Exception as e:
		logger.error(f"Error in Instagram file creation: {e}")
		print(f"[!] Error: {e}")
		return
...
def Facebook():
	try:
		...
		if not Target.strip():
			print(alert + " [!] Error: Target name cannot be empty")
			return
			
		TargetEmail = input(start + " Enter Target Email: " + white)
		if not TargetEmail.strip():
			print(alert + " [!] Error: Target email cannot be empty")
			return
			
		phishURL = input(start + " Enter Phishing URL: " + white)
		if not phishURL.strip():
			print(alert + " [!] Error: Phishing URL cannot be empty")
			return
		if not phishURL.startswith(('http://', 'https://')):
			print(alert + " [!] Error: URL must start with http:// or https://")
			return
			
		Date = int(input(start + " Enter a number as date: " + white))
	except ValueError:
		logger.error("Invalid date input in Facebook")
		print("\n[!] Error: Please enter a valid number for date")
		return
	except KeyboardInterrupt:
		logger.info("User interrupted Facebook function")
		print("\nOperation cancelled by user")
		return
	except EOFError:
		logger.error("EOF encountered in Facebook function")
		print("\n[!] Error: Input interrupted")
		return
	except Exception as e:
		logger.error(f"Error in Facebook input: {e}")
		print(f"[!] Error: {e}")
		return
	...
	try:
		...
		monthpick = int(input(green + "root@BlazeZephyr/month:~ "))
		month = monthName(monthpick)
	except ValueError:
		logger.error("Invalid month selection in Facebook")
		print("\n[!] Error: Please enter a valid month number")
		return
	except KeyboardInterrupt:
		logger.info("User interrupted month selection")
		print("\nOperation cancelled by user")
		return
	except EOFError:
		logger.error("EOF encountered during month selection")
		print("\n[!] Error: Input interrupted")
		return
	except Exception as e:
		logger.error(f"Error in month selection: {e}")
		print(f"[!] Error: {e}")
		return
	...
	try:
		...
		if not all([city.strip(), country.strip(), timelog.strip()]):
			print(alert + " [!] Error: All fields are required")
			return
	except ValueError:
		logger.error("Invalid year input in Facebook")
		print("\n[!] Error: Please enter a valid year")
		return
	except KeyboardInterrupt:
		logger.info("User interrupted Facebook details input")
		print("\nOperation cancelled by user")
		return
	except EOFError:
		logger.error("EOF encountered during Facebook details input")
		print("\n[!] Error: Input interrupted")
		return
	except Exception as e:
		logger.error(f"Error in Facebook details input: {e}")
		print(f"[!] Error: {e}")
		return
...
def Gmail():
	...
	monthpick = int(input(green + "root@CyberPhish:~ " + white))
...
def RiotGames():
	...
	print("")
	ServerPick = int(input("root@CyberPhish/RiotGames/Server:~ "))
